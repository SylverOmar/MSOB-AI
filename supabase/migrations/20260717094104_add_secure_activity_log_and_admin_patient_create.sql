...
create table private.activity_log (
  ...
  actor_type text not null check (actor_type in ('doctor', 'admin')),
  actor_id uuid not null,
  actor_access_id text not null,
  actor_prenom text not null,
  actor_nom text not null,
  ...
  target_type text not null check (
    target_type in ('patient', 'doctor', 'medical_folder', 'report', 'analysis')
  ),
  target_id text,
  target_label_encrypted text,
  details jsonb not null default '{}'::jsonb check (jsonb_typeof(details) = 'object')
);

alter table private.activity_log enable row level security;

create index activity_log_created_at_idx
  on private.activity_log (created_at desc);

create index activity_log_actor_idx
  on private.activity_log (actor_type, actor_id, created_at desc);

create index activity_log_target_idx
  on private.activity_log (target_type, target_id, created_at desc);

revoke all on table private.activity_log
...
-- Current Supabase guidance for SECURITY DEFINER functions is an empty
-- search_path with every non-pg_catalog object schema-qualified. These
-- existing functions already qualify their database objects.
alter function public.msob_gateway(text, text, jsonb) set search_path = '';
alter function public.msob_admin_patient_gateway(text, text, jsonb) set search_path = '';
alter function public.secure_create_patient(text, text, text, integer, character varying, character varying) set search_path = '';
alter function public.secure_update_patient(uuid, text, text, text, integer, character varying, character varying) set search_path = '';
alter function public.secure_patient_list() set search_path = '';

comment on table private.activity_log is
  'Application audit trail. Patient/doctor target labels are encrypted and readable only through the admin-only server boundary.';
